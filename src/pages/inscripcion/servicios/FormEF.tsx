...
import type { DetalleEF, DocumentoSeleccionado, DetallesServicios, Canal } from '../servicios-types';
...
const valorInicial = (): DetalleEF => ({
  envioDocumentos: 'acepta',
  documentos: [],
...
type Props = {
  value?: DetalleEF;
  onChange: (v: DetalleEF) => void;
  detalles: DetallesServicios;
  seleccionados: string[];
};

export function FormEF({ value, onChange, seleccionados }: Props) {
  const v = {
    ...valorInicial(),
    ...value,
  ...
  function actualizar(cambios: Partial<DetalleEF>) {
    onChange({ ...v, ...cambios });
  }
  ...
  function toggleDocumento(codigo: string) {
    ...
    if (existe) {
      ...
      const nuevo: DocumentoSeleccionado = { codigo, almacenaEn: ['EF'], canales: ['nativo'] };
      actualizar({ documentos: [...v.documentos, nuevo] });
    }
  }

  function toggleAlmacenamiento(codigo: string, lugar: 'EF' | 'BO') {
  ...
  return (
    <Box>
      {/* Envío de Documentos */}
      <Typography variant="subtitle2" sx={{ mb: 1 }}>Envío de Documentos</Typography>
      <RadioGroup
        row
        value={v.envioDocumentos}
        onChange={(e) => actualizar({ envioDocumentos: e.target.value as 'acepta' | 'bypass' })}
        sx={{ mb: 3 }}
      >
        <FormControlLabel value="acepta" control={<Radio size="small" />} label="Acepta" />
        <FormControlLabel value="bypass" control={<Radio size="small" />} label="Bypass (BackOffice)" />
      </RadioGroup>
          ...
          {v.documentos.map((doc) => (
            <Box key={doc.codigo} sx={{ mb: 2, ml: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ border: '1px solid #EAE5F5', borderRadius: '8px', p: 1.2, bgcolor: '#FAF8FD', flex: '1 1 220px' }}>
                  ...
                  {tieneBO ? (
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      <FormControlLabel
                        control={<Checkbox size="small" checked={doc.almacenaEn.includes('EF')} onChange={() => toggleAlmacenamiento(doc.codigo, 'EF')} />}
                        label={<Typography sx={{ fontSize: 12 }}>EF</Typography>}
                      />
                      <FormControlLabel
                    ...
                    <Typography sx={{ fontSize: 11.5, color: 'text.disabled' }}>EF (no tiene BO contratado)</Typography>
                  )}
                </Box>
                ...
                {/* Bloque: Modo de firma — solo si NO se emite por el canal nativo EF */}
      ...
      {/* Modo de firma — fijo, solo si algún documento se emite nativo */}
      {tieneEmisionNativa && (
        <Typography sx={{ fontSize: 12.5, color: 'text.secondary', mb: 2 }}>
          Modo de firma (emisión nativa EF): <strong>Controlada</strong>
        </Typography>
      )}
