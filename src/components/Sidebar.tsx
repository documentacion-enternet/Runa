...
import {
  ...
  DialogContent, DialogActions, TextField, Alert, Button, Badge,
} from '@mui/material';
...
import { useEffect, useState as useStateCount } from 'react';
...
export function Sidebar() {
  ...
  const { perfil, session, cerrarSesion, esVista, puedeGestionar, esAdmin } = useAuth();
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('completado', false)
      .then(({ count }) => setCountMisEmpresas(count ?? 0));
  }, [session?.user.id]);

  async function handleGuardarContrasena() {
    setErrorContrasena(null);
    if (nuevaContrasena.length < 8) { setErrorContrasena('La contraseña debe tener al menos 8 caracteres'); return; }
    if (nuevaContrasena !== confirmarContrasena) { setErrorContrasena('Las contraseñas no coinciden'); return; }
    setGuardandoContrasena(true);
    const { error } = await supabase.auth.updateUser({ password: nuevaContrasena });
    setGuardandoContrasena(false);
    if (error) { setErrorContrasena(error.message); return; }
  ...
  function cerrarDialogoContrasena() {
    ...
    setErrorContrasena(null); setExitoContrasena(false);
  }
  ...
  const navItems = [
    ...
    {
      label: 'Mis Empresas', path: '/mis-empresas',
      icon: (
        <Badge badgeContent={countMisEmpresas} color="primary" max={99}
    ...
    ...(puedeGestionar ? [
      { label: 'Importar contactos', path: '/importar-contactos-csv', icon: <ContactsOutlinedIcon fontSize="small" /> },
    ] : []),
    ...(puedeGestionar ? [
  ...
  return (
    <Drawer
      ...
      sx={{
        ...
        '& .MuiDrawer-paper': { width: SIDEBAR_WIDTH, boxSizing: 'border-box', borderRight: '1px solid #EAE5F5', bgcolor: 'background.paper' },
      }}
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2.5, py: 2.5 }}>
        <Box sx={{ width: 32, height: 32, borderRadius: '8px', bgcolor: 'text.primary', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <List sx={{ px: 1.5, py: 2, flexGrow: 1 }}>
        {navItems.map((item) => {
          ...
          return (
            <ListItemButton key={item.path} selected={activo} onClick={() => navigate(item.path)}
              sx={{
                borderRadius: '8px', mb: 0.5,
                '&.Mui-selected': { bgcolor: 'rgba(122, 107, 176, 0.08)', color: 'primary.main', '& .MuiListItemIcon-root': { color: 'primary.main' } },
      ...
      <Box onClick={(e) => setAnchorEl(e.currentTarget)}
        sx={{ display: 'flex', alignItems: 'center', gap: 1.2, px: 2, py: 1.8, cursor: 'pointer', '&:hover': { bgcolor: 'rgba(122, 107, 176, 0.05)' } }}>
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
        <MenuItem onClick={() => { setAnchorEl(null); setDialogoContrasenaAbierto(true); }} sx={{ fontSize: 13.5, gap: 1.2 }}>
          <LockOutlinedIcon fontSize="small" sx={{ color: 'text.secondary' }} /> Cambiar contraseña
        </MenuItem>
        <MenuItem onClick={handleLogout} sx={{ fontSize: 13.5, gap: 1.2 }}>
          <LogoutIcon fontSize="small" sx={{ color: 'error.main' }} /> Cerrar sesión
        </MenuItem>
      </Menu>
        ...
        <DialogContent>
          {exitoContrasena ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 0.5 }}>
              ...
              <TextField label="Nueva contraseña" type="password" fullWidth autoFocus value={nuevaContrasena} onChange={(e) => setNuevaContrasena(e.target.value)} helperText="Mínimo 8 caracteres" />
              <TextField label="Confirmar contraseña" type="password" fullWidth value={confirmarContrasena} onChange={(e) => setConfirmarContrasena(e.target.value)} />
            </Box>
          )}
        </DialogContent>
